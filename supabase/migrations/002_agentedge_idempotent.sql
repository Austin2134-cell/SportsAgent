-- AgentEdge idempotent migration — safe to re-run (handles "already exists" errors)
-- Run this in Supabase SQL Editor if 001 failed partway or policies already exist
...
-- ── RLS (drop + recreate so re-run never fails) ─────────────────────────────
...
DROP POLICY IF EXISTS "Users can view own agent" ON agent_instances;
CREATE POLICY "Users can view own agent" ON agent_instances
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can view own episodes" ON agent_episodes;
CREATE POLICY "Users can view own episodes" ON agent_episodes
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can view own beliefs" ON agent_beliefs;
CREATE POLICY "Users can view own beliefs" ON agent_beliefs
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can view own hypotheses" ON agent_hypotheses;
