...
def _build_wc_user_message(
    ...
    parts.append(
        ...
        "Evaluate each game with an open mind — ML, DNB, Over 2.5, Under 2.5, BTTS, and "
        "handicaps are all in play. Use WC historical averages as reference context only; "
        "do not default to any market type. Pick the best edge per game regardless of category. "
        "Juice ceiling -150 applies absolutely (no play worse than -150). "
        "Flag dead rubbers and rotation risk as automatic passes.\n"
        "Decision order: fundamentals → market intelligence (steam/reverse/splits) → posted price "
        "for implied_prob only. Splits confirm or fade your read — never pick Over or Under "
        "solely because of public betting %.\n"
...
def run_world_cup_card(
    *,
    ...
    if active_db is not None:
        ...
        splits_result = run_splits_sync(active_db)
