...
class StatsClient:
    ...
    def get_scoreboard(self, odds_sport_key: str, date_str: str = None) -> list[dict]:
        ...
        Returns list of game summaries with team stats.
        """
    ...
    def get_player_recent_stats(
        ...
        """
        Fetch recent game log for a player by name.
        Searches the ESPN athlete finder, then pulls game log.
        Returns last N games of stats or None if not found.
        """
        sport, league = SPORT_MAP.get(odds_sport_key, (None, None))
        if not sport:
            return None
    ...
    def _find_athlete_id(self, sport: str, league: str, name: str) -> Optional[str]:
        """Search ESPN for an athlete by name and return their ID."""
        data = self._get(
    ...
    def _get_game_log(
        ...
        """Fetch game log for an athlete from ESPN."""
        data = self._get(
    ...
    def build_context_package(
        ...
        """
        Build a full context package for one sport:
        - Today's games / teams
